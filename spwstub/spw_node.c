...
void create_pipes(struct SpWInterface* const spw_int, pipe_fd tx, pipe_fd rx) {
    ...
    printf("[PARENT] %2d <- %2d [TX]\n", inner_tx_to_parent[0], inner_tx_to_parent[0]);
    ...
    printf("[PARENT] %2d <- %2d [RX]\n", inner_rx_to_parent[0], inner_rx_to_parent[0]);
...
void create_forks(struct SpWInterface* const spw_int) {
    if(fork() == 0){
...
int32_t wait_all_stop(struct SpWInterface* const spw_int) {
    pipe_fd read_fd[3] = {spw_int->from_console_read, spw_int->from_rx_read, spw_int->from_tx_read};
    bool is_stop[3] = {false, false, true};
    uint8_t stoped = 1;
    while(stoped < 3) {
...
int32_t poll_children(struct SpWInterface* const spw_int, Message* msg) {
...
int32_t parent_duty(struct SpWInterface* const spw_int) {
